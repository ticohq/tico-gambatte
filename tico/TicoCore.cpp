/// @file TicoCore.cpp
/// @brief Simplified libretro frontend for gambatte with tico overlay
/// N64: no disk control, ROM loaded into memory (need_fullpath=false),
/// HW render via GLSM, save data uses native Gambatte formats with .srm fallback
...
// Forward declarations for gambatte core functions (C linkage)
extern "C"
{
...
static uint32_t RC_CCONV RAReadMemory(uint32_t address, uint8_t* buffer, uint32_t num_bytes, rc_client_t* client)
{
    ...
    if (sram && address >= 0xA000 && (address - 0xA000) + num_bytes <= sram_size) {
        memcpy(buffer, sram + (address - 0xA000), num_bytes);
        return num_bytes;
    }
...
bool TicoCore::Init()
{
    ...
    // Ensure system/Gambatteplus directories exist
    struct stat st = {0};
    if (stat(m_systemDir.c_str(), &st) == -1) {
        mkdir(m_systemDir.c_str(), 0777);
    }
    std::string gambatteDir = m_systemDir + "Gambatteplus/";
    if (stat(gambatteDir.c_str(), &st) == -1) {
        mkdir(gambatteDir.c_str(), 0777);
    }
...
bool TicoCore::HandleEnvironment(unsigned cmd, void *data)
{
    ...
    //==================================================================
    // Additional environment commands required by gambatte-Next
    //==================================================================
...
#ifdef __SWITCH__
    configPath = "sdmc:/tico/config/cores/gambatte.jsonc";
#else
    configPath = "tico/config/cores/gambatte.jsonc";
#endif
