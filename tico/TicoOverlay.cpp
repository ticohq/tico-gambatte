...
/// @brief Overlay UI for tico-integrated gambatte (no disc support)
...
static std::string GetStatePath(TicoCore *core, int slot) {
  ...
  return std::string(TicoConfig::STATES_PATH) + romName + ".state" +
         std::to_string(slot);
}
...
inline void DrawTextWithShadow(ImDrawList *dl, ImVec2 pos, ImU32 color,
  ...
  dl->AddText(ImVec2(pos.x + shadowOffset, pos.y + shadowOffset),
              IM_COL32(0, 0, 0, 50), text);
  dl->AddText(pos, color, text);
}
static void DrawSwitchButton(ImDrawList *dl, ImFont *font, float fontSize,
                             ImVec2 center, float size, const char *symbol,
                             float alpha, bool isDark) {
...
void TicoOverlay::LoadConfig() {
  const char *configPaths[] = {"sdmc:/tiicu/config/display.jsonc",
                               "sdmc:/tico/config/display.jsonc",
...
void TicoOverlay::LoadGeneralConfig() {
  const char *configPaths[] = {"sdmc:/tiicu/config/general.jsonc",
  ...
  if (fp) {
    ...
    if (size > 0) {
      ...
      fread(&content[0], 1, size, fp);
      auto j = nlohmann::json::parse(content, nullptr, false, true);
      if (!j.is_discarded() && j.contains("hour_format") &&
          j["hour_format"].is_string())
...
void TicoOverlay::LoadAccountData() {
  ...
  if (R_SUCCEEDED(accountGetPreselectedUser(&uid)) && accountUidIsValid(&uid))
    found = true;
  if (!found && R_SUCCEEDED(accountGetLastOpenedUser(&uid)) &&
      accountUidIsValid(&uid))
    found = true;
  if (!found) {
    s32 userCount = 0;
    if (R_SUCCEEDED(accountGetUserCount(&userCount)) && userCount > 0) {
      ...
      s32 actualTotal = 0;
      if (R_SUCCEEDED(
              accountListAllUsers(uids, ACC_USER_LIST_SIZE, &actualTotal)) &&
          actualTotal > 0) {
        uid = uids[0];
        found = true;
      }
  ...
  if (found) {
    ...
    if (R_SUCCEEDED(accountGetProfile(&profile, uid))) {
      ...
      if (R_SUCCEEDED(accountProfileGetImageSize(&profile, &imageSize)) &&
        ...
        if (jpegBuf) {
          u32 actualSize = 0;
          if (R_SUCCEEDED(accountProfileLoadImage(&profile, jpegBuf, imageSize,
                                                  &actualSize))) {
            int width, height, channels;
            unsigned char *rgba = stbi_load_from_memory(
                jpegBuf, actualSize, &width, &height, &channels, 4);
            if (rgba) {
              if (m_avatarTexture != 0)
                glDeleteTextures(1, &m_avatarTexture);
              glGenTextures(1, &m_avatarTexture);
...
void TicoOverlay::Render(ImVec2 displaySize, unsigned int gameTexture,
                         float aspectRatio, int frameWidth, int frameHeight,
                         int fboWidth, int fboHeight) {
  ImDrawList *bgDrawList = ImGui::GetBackgroundDrawList();
  ImDrawList *fgDrawList = ImGui::GetForegroundDrawList();
  RenderGame(bgDrawList, displaySize, gameTexture, aspectRatio, frameWidth,
             frameHeight, fboWidth, fboHeight);
  if (m_currentMenu != OverlayMenu::None) {
...
void TicoOverlay::RenderRAAlerts(ImDrawList *dl, ImVec2 displaySize,
                                 float deltaTime) {
  if (!m_core)
    return;
  ...
  for (size_t i = 0; i < notifications.size(); i++) {
    ...
    ImU32 bgColor = m_isDarkMode ? IM_COL32(35, 35, 40, alpha)
                                 : IM_COL32(245, 248, 252, alpha);
    ImU32 borderColor =
        m_isDarkMode ? IM_COL32(70, 70, 80, (int)(180 * slideProgress))
    ...
    // Description text
    ImU32 descColor = m_isDarkMode ? IM_COL32(185, 185, 195, alpha)
                                   : IM_COL32(80, 80, 95, alpha);
    float maxDescW = rectMax.x - textX - padding;

    ImU32 titleColor = m_isDarkMode ? IM_COL32(255, 255, 255, alpha)
...
void TicoOverlay::RenderSocialArea(ImDrawList *dl, ImVec2 displaySize) {
  ...
  if (ease < 0.01f)
    return;
  float scale = ImGui::GetIO().FontGlobalScale;
  float AVATAR_SIZE = 72.0f * scale, sideMargin = 32.0f * scale,
        topMargin = 32.0f * scale, barHeight = 50.0f * scale;
  ...
  ImU32 baseCol = m_isDarkMode ? IM_COL32(45, 45, 45, (int)(255 * ease))
                               : IM_COL32(245, 247, 250, (int)(200 * ease));
  dl->AddCircleFilled(avatarCenter, radius, baseCol);
  if (m_avatarTexture != 0) {
    float imgRadius = radius - 4.0f;
    dl->AddImageRounded(
        (ImTextureID)(intptr_t)m_avatarTexture,
        ImVec2(avatarCenter.x - imgRadius, avatarCenter.y - imgRadius),
        ImVec2(avatarCenter.x + imgRadius, avatarCenter.y + imgRadius),
        ImVec2(0, 0), ImVec2(1, 1), IM_COL32_WHITE, imgRadius);
    dl->AddCircle(avatarCenter, imgRadius, IM_COL32(255, 255, 255, 60), 0,
                  1.0f);
  } else {
    dl->AddCircleFilled(avatarCenter, radius - 4.0f,
                        IM_COL32(200, 200, 210, 255));
  }
}

void TicoOverlay::RenderGame(ImDrawList *dl, ImVec2 displaySize,
                             unsigned int texture, float aspectRatio, int width,
  ...
  if (m_displayMode == GambatteDisplayMode::Integer) {
    ...
    case GambatteDisplaySize::Stretch:
      dstWidth = displaySize.x;
      dstHeight = displaySize.y;
      break;
...
void TicoOverlay::RenderOverlayBackground(ImDrawList *dl, ImVec2 displaySize) {
  ...
  if (baseAlpha > 0) {
    ...
    ImU32 colMax = IM_COL32(0, 0, 0, maxAlpha),
          colBase = IM_COL32(0, 0, 0, baseAlpha);
    dl->AddRectFilledMultiColor(ImVec2(0, 0), ImVec2(displaySize.x, topH),
                                colMax, colMax, colBase, colBase);
    dl->AddRectFilled(ImVec2(0, topH),
                      ImVec2(displaySize.x, displaySize.y - botH), colBase);
    dl->AddRectFilledMultiColor(ImVec2(0, displaySize.y - botH),
                                ImVec2(displaySize.x, displaySize.y), colBase,
                                colBase, colMax, colMax);
  }
}
...
void TicoOverlay::RenderTitleCard(ImDrawList *dl, ImVec2 displaySize) {
  ...
  if (m_currentMenu == OverlayMenu::SaveStates)
    titleStr =
        m_isSaveMode ? tr("emulator_save_state") : tr("emulator_load_state");
  ...
  float cardWidth = displaySize.x * 0.4f,
        cardX = (displaySize.x - cardWidth) * 0.5f,
        cardY = (AVAILABLE_TOP - TITLE_HEIGHT) * 0.5f;
  float t = std::min(m_animTimer / 0.4f, 1.0f);
  float easeOut = 1.0f - std::pow(1.0f - t, 3.0f);
  float startY = -150.0f * scale,
        currentY = startY + (cardY - startY) * easeOut;
  ImVec2 textSize = ImGui::CalcTextSize(titleStr.c_str());
  float textX = cardX + (cardWidth - textSize.x) * 0.5f,
        textY = currentY + (TITLE_HEIGHT - textSize.y) * 0.5f;
  UIStyle::DrawTextWithShadow(dl, ImVec2(textX, textY),
                              IM_COL32(200, 200, 200, 255), titleStr.c_str());
}

/// @brief Render an animated menu container with rounded corners
static void RenderMenuContainer(ImDrawList *dl, ImVec2 displaySize,
                                float menuWidth, int numItems, float itemHeight,
                                float animTimer, bool isDark, ImVec2 &menuPos,
                                ImVec2 &menuSize, float &easeOut,
                                float &cornerRadius) {
  float scale = ImGui::GetIO().FontGlobalScale;
  ...
  float targetY = (displaySize.y - menuSize.y) / 2.0f,
        startY = displaySize.y + 100.0f * scale;
  menuPos = ImVec2((displaySize.x - menuSize.x) / 2,
                   startY + (targetY - startY) * easeOut);
  cornerRadius = 16.0f * scale;
  ImU32 containerColor = isDark ? IM_COL32(45, 45, 45, (int)(255 * easeOut))
                                : IM_COL32(242, 245, 248, (int)(255 * easeOut));
  dl->AddRectFilled(menuPos,
                    ImVec2(menuPos.x + menuSize.x, menuPos.y + menuSize.y),
                    containerColor, cornerRadius);
}

static void RenderMenuItem(ImDrawList *dl, ImVec2 menuPos, ImVec2 menuSize,
                           int i, int numItems, float itemHeight,
                           bool isSelected, float cornerRadius, float easeOut,
                           bool isDark, ImFont *font, float fontSize,
                           const char *text) {
  float scale = ImGui::GetIO().FontGlobalScale;
  float itemY = menuPos.y + i * itemHeight;
  ImVec2 itemMin(menuPos.x, itemY),
      itemMax(menuPos.x + menuSize.x, itemY + itemHeight);
  if (isSelected) {
    ...
    ImU32 selCol = isDark ? IM_COL32(60, 60, 60, (int)(255 * easeOut))
                          : IM_COL32(190, 195, 205, (int)(255 * easeOut));
    dl->AddRectFilled(itemMin, itemMax, selCol, itemRadius, corners);
  }
  ImU32 textColor;
  if (isDark)
    textColor = isSelected ? IM_COL32(255, 255, 255, (int)(255 * easeOut))
                           : IM_COL32(200, 200, 200, (int)(255 * easeOut));
  else
    textColor = isSelected ? IM_COL32(60, 60, 70, (int)(255 * easeOut))
                           : IM_COL32(90, 90, 100, (int)(255 * easeOut));
  ImVec2 sz = font->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, text);
  dl->AddText(
      font, fontSize,
      ImVec2(itemMin.x + 20.0f * scale, itemMin.y + (itemHeight - sz.y) / 2),
      textColor, text);
}

void TicoOverlay::RenderQuickMenu(ImDrawList *dl, ImVec2 displaySize) {
  float scale = ImGui::GetIO().FontGlobalScale;
  std::string items[] = {tr("emulator_save_state"), tr("emulator_load_state"),
                         tr("emulator_settings"), tr("emulator_exit_game")};
  const int N = 4;
  float itemH = 64.0f * scale;
  ImVec2 menuPos, menuSize;
  float easeOut, cornerRadius;
  RenderMenuContainer(dl, displaySize, 400.0f * scale, N, itemH, m_animTimer,
                      m_isDarkMode, menuPos, menuSize, easeOut, cornerRadius);
  ImFont *font = ImGui::GetFont();
  float fs = ImGui::GetFontSize() * 0.85f;
  for (int i = 0; i < N; i++)
    RenderMenuItem(dl, menuPos, menuSize, i, N, itemH,
                   m_quickMenuSelection == i, cornerRadius, easeOut,
                   m_isDarkMode, font, fs, items[i].c_str());
}
...
void TicoOverlay::RenderSaveStatesMenu(ImDrawList *dl, ImVec2 displaySize) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (m_core && m_core->IsGameLoaded()) {
      struct stat buffer;
      exists = (stat(GetStatePath(m_core, i).c_str(), &buffer) == 0);
    }
    char slotText[128];
    snprintf(slotText, sizeof(slotText), tr("emulator_slot").c_str(), i + 1,
             exists ? tr("emulator_in_use").c_str()
                    : tr("emulator_empty").c_str());
    RenderMenuItem(dl, menuPos, menuSize, i, N, itemH, m_saveStateSlot == i,
                   cornerRadius, easeOut, m_isDarkMode, font, fs, slotText);
  }
}
...
void TicoOverlay::RenderSettingsMenu(ImDrawList *dl, ImVec2 displaySize) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    ImVec2 itemMin(menuPos.x, itemY),
        itemMax(menuPos.x + menuSize.x, itemY + itemH);
    if (isSelected) {
      ImDrawFlags corners = 0;
      float itemRadius = 0.0f;
      if (i == 0) {
      ...
      ImU32 selCol = m_isDarkMode
                         ? IM_COL32(60, 60, 60, (int)(255 * easeOut))
                         : IM_COL32(190, 195, 205, (int)(255 * easeOut));
      dl->AddRectFilled(itemMin, itemMax, selCol, itemRadius, corners);
    }
    ...
    if (i == 0) {
      ...
      value = (m_displayMode == GambatteDisplayMode::Integer)
                  ? tr("emulator_integer")
                  : tr("emulator_display");
    } else if (i == 1) {
      ...
      if (m_displayMode == GambatteDisplayMode::Integer) {
        switch (m_displaySize) {
        case GambatteDisplaySize::_1x:
          value = "1x";
          break;
        case GambatteDisplaySize::_2x:
          value = "2x";
          break;
        default:
      ...
      const char *shaderNames[] = {"None",  "LCD", "xBRZ",
                                   "Eagle", "Dot", "LCD Grid V2"};
    ...
    dl->AddText(font, fs, ImVec2(textX, itemMin.y + (itemH - labelSize.y) / 2),
    ...
    dl->AddText(font, fs, ImVec2(valueX, itemMin.y + (itemH - labelSize.y) / 2),
                textColor, value.c_str());
    if (isSelected) {
      float arrowSize = 12.0f * scale,
            arrowY = itemMin.y + (itemH - arrowSize) / 2;
      float lx = valueX - arrowSize - 12.0f * scale;
      dl->AddTriangleFilled(
          ImVec2(lx, arrowY + arrowSize / 2), ImVec2(lx + arrowSize, arrowY),
          ImVec2(lx + arrowSize, arrowY + arrowSize), textColor);
      float rx = valueX + valueSize.x + 12.0f * scale;
      dl->AddTriangleFilled(ImVec2(rx + arrowSize, arrowY + arrowSize / 2),
                            ImVec2(rx, arrowY), ImVec2(rx, arrowY + arrowSize),
                            textColor);
    }
  }
}
...
void TicoOverlay::RenderHelpersBar(ImDrawList *dl, ImVec2 displaySize) {
  ...
  float easeOut = 1.0f - std::pow(1.0f - t, 3.0f);
  float scale = ImGui::GetIO().FontGlobalScale;
  float BAR_HEIGHT = 48.0f * scale, MARGIN_BOTTOM = 24.0f * scale,
        PADDING = 16.0f * scale, BUTTON_SIZE = 22.0f * scale,
        ITEM_SPACING = 12.0f * scale;
  ImFont *font = ImGui::GetFont();
  ...
  for (size_t i = 0; i < helpers.size(); i++) {
    totalWidth +=
        BUTTON_SIZE + 8.0f * scale +
        font->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, helpers[i].desc.c_str()).x;
    if (i < helpers.size() - 1)
      totalWidth += ITEM_SPACING;
  }
  float startOffset = 400.0f * scale,
        currentOffset = startOffset * (1.0f - easeOut);
  float barX = displaySize.x - totalWidth - 20.0f + currentOffset,
        barY = displaySize.y - MARGIN_BOTTOM - BAR_HEIGHT;
  float cursorX = barX + PADDING, centerY = barY + BAR_HEIGHT * 0.5f;
  for (const auto &h : helpers) {
    UIStyle::DrawSwitchButton(dl, font, fontSize,
                              ImVec2(cursorX + BUTTON_SIZE * 0.5f, centerY),
                              BUTTON_SIZE, h.btn, easeOut, true);
    cursorX += BUTTON_SIZE + 8.0f * scale;
    ImVec2 textSize =
        font->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, h.desc.c_str());
    dl->AddText(font, fontSize, ImVec2(cursorX, centerY - textSize.y * 0.5f),
                IM_COL32(200, 200, 200, (int)(255 * easeOut)), h.desc.c_str());
...
bool TicoOverlay::HandleInput(SDL_GameController *controller) {
  ...
  if (up && !m_upHeld && debounced) {
    ...
    m_lastInputTime = now;
    if (m_currentMenu == OverlayMenu::QuickMenu)
      m_quickMenuSelection = (m_quickMenuSelection + 3) % 4;
    else if (m_currentMenu == OverlayMenu::SaveStates)
      m_saveStateSlot = (m_saveStateSlot + 3) % 4;
    else if (m_currentMenu == OverlayMenu::Settings)
      m_settingsSelection = (m_settingsSelection + 2) % 3;
  }
  if (!up)
    m_upHeld = false;
  ...
  if (dirChanged && m_currentMenu == OverlayMenu::Settings) {
    if (m_settingsSelection == 0) {
      m_displayMode = (m_displayMode == GambatteDisplayMode::Display)
                          ? GambatteDisplayMode::Integer
                          : GambatteDisplayMode::Display;
      m_displaySize = (m_displayMode == GambatteDisplayMode::Integer)
                          ? GambatteDisplaySize::Auto
                          : GambatteDisplaySize::_4_3;
      ApplyScalingSettings(true);
    } else if (m_settingsSelection == 1) {
      if (m_displayMode == GambatteDisplayMode::Integer) {
        int s = (int)m_displaySize + dir;
        if (s < 4)
          s = 6;
        if (s > 6)
          s = 4;
...
#ifdef __SWITCH__
  std::string configPath = "sdmc:/tico/config/cores/gambatte.jsonc";
#else
  std::string configPath = "tico/config/cores/gambatte.jsonc";
#endif
  ...
  if (file.is_open()) {
    ...
    if (!j.is_discarded()) {
      if (j.contains("display_mode") && j["display_mode"].is_string()) {
        m_displayMode = (j["display_mode"].get<std::string>() == "Integer")
                            ? GambatteDisplayMode::Integer
                            : GambatteDisplayMode::Display;
      } else
      ...
      if (j.contains("display_size") && j["display_size"].is_string()) {
        ...
        if (v == "Stretch")
          m_displaySize = GambatteDisplaySize::Stretch;
        else if (v == "16:9")
          m_displaySize = GambatteDisplaySize::_16_9;
        else if (v == "Original")
          m_displaySize = GambatteDisplaySize::Original;
        else if (v == "1x")
          m_displaySize = GambatteDisplaySize::_1x;
        else if (v == "2x")
          m_displaySize = GambatteDisplaySize::_2x;
        else if (v == "Auto")
...
void TicoOverlay::SaveCoreSettings() {
  ...
  nlohmann::json j = nlohmann::json::object();
  {
    ...
    if (in.is_open()) {
      auto p = nlohmann::json::parse(in, nullptr, false, true);
      in.close();
      if (!p.is_discarded())
        j = p;
    }
  ...
  j["display_mode"] =
      (m_displayMode == GambatteDisplayMode::Integer) ? "Integer" : "Display";
  ...
  case GambatteDisplaySize::_16_9:
    sizeStr = "16:9";
    break;
  case GambatteDisplaySize::Original:
    sizeStr = "Original";
    break;
  case GambatteDisplaySize::_1x:
    sizeStr = "1x";
...
void TicoOverlay::LoadSVGIcon() {
  const char *svgContent =
      R"(<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="#FFFFFF" d="M338.8-9.9c11.9 8.6 16.3 24.2 10.9 37.8L271.3 224 416 224c13.5 0 25.5 8.4 30.1 21.1s.7 26.9-9.6 35.5l-288 240c-11.3 9.4-27.4 9.9-39.3 1.3s-16.3-24.2-10.9-37.8L176.7 288 32 288c-13.5 0-25.5-8.4-30.1-21.1s-.7-26.9 9.6-35.5l288-240c11.3-9.4 27.4-9.9 39.3-1.3z"/></svg>)";
...
void TicoOverlay::RenderStatusBar(ImDrawList *dl, ImVec2 displaySize) {
  ...
  float scale = ImGui::GetIO().FontGlobalScale;
  float BAR_HEIGHT = 50.0f * scale, TOP_MARGIN = 32.0f * scale,
        SIDE_MARGIN = 32.0f * scale, ITEM_SPACING = 20.0f * scale;
  ImFont *font = ImGui::GetFont();
  float fontSize = ImGui::GetFontSize();
  std::time_t now = std::time(nullptr);
  std::tm *lt = std::localtime(&now);
  char timeStr[16], periodStr[16];
  bool is24h = (m_hourFormat == "24h");
  float timeW = 0, periodFontSize = fontSize * 0.55f, periodW = 0;
  if (is24h) {
    std::strftime(timeStr, sizeof(timeStr), "%H:%M", lt);
    timeW = font->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, timeStr).x;
    periodStr[0] = 0;
  } else {
    std::strftime(timeStr, sizeof(timeStr), "%I:%M", lt);
    std::strftime(periodStr, sizeof(periodStr), "%p", lt);
    timeW = font->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, timeStr).x;
  ...
  totalWidth += ITEM_SPACING + 34.0f * scale;
  float PADDING = 20.0f * scale;
  totalWidth += PADDING * 2;
  float offsetY = (1.0f - ease) * -20.0f;
  float barX = displaySize.x - totalWidth - SIDE_MARGIN,
        barY = TOP_MARGIN + offsetY;
  ImU32 textColor = IM_COL32(200, 200, 200, (int)(255 * alpha));
  float cursorX = barX + PADDING, centerY = barY + BAR_HEIGHT * 0.5f;
  dl->AddText(font, fontSize, ImVec2(cursorX, centerY - fontSize * 0.5f),
              textColor, timeStr);
  cursorX += timeW;
  if (!is24h) {
    cursorX += 4.0f * scale;
    dl->AddText(font, periodFontSize,
                ImVec2(cursorX, centerY - fontSize * 0.5f +
                                    (fontSize - periodFontSize) * 0.9f),
                textColor, periodStr);
    cursorX += periodW;
  }
  cursorX += ITEM_SPACING;
  float bodyW = 32.0f * scale, bodyH = 20.0f * scale, tipW = 4.0f * scale,
        tipH = 10.0f * scale;
  ...
  ImVec2 tipMin(bodyMax.x, batteryPos.y + (bodyH - tipH) * 0.5f),
      tipMax = tipMin + ImVec2(tipW, tipH);
  dl->AddRectFilled(tipMin, tipMax, textColor, 2.0f,
                    ImDrawFlags_RoundCornersRight);
  float pct = std::clamp(m_batteryLevel / 100.0f, 0.0f, 1.0f);
  float pad = 4.0f * scale, fillMaxW = bodyW - pad * 2,
        currentFillW = fillMaxW * pct;
  if (currentFillW < 2.0f * scale && pct > 0)
    currentFillW = 2.0f * scale;
  if (currentFillW > 0)
    dl->AddRectFilled(bodyMin + ImVec2(pad, pad),
                      bodyMin + ImVec2(pad + currentFillW, bodyH - pad),
                      textColor, 1.0f);
  if (m_isCharging) {
    if (m_boltTexture == 0)
      LoadSVGIcon();
    if (m_boltTexture != 0) {
      float iconH = 16.0f * scale,
            iconW = iconH * ((float)m_boltWidth / (float)m_boltHeight);
      ImVec2 iconPos(tipMax.x + 6.0f * scale,
                     batteryPos.y + (bodyH - iconH) * 0.5f);
      float fadeProgress =
          std::max(0.0f, (m_chargingStateProgress - 0.5f) * 2.0f);
      int alphaBolt = (int)(255 * fadeProgress * ease);
      if (alphaBolt > 0)
        dl->AddImage((ImTextureID)(intptr_t)m_boltTexture, iconPos,
                     iconPos + ImVec2(iconW, iconH), ImVec2(0, 0), ImVec2(1, 1),
                     IM_COL32(235, 235, 235, alphaBolt));
    }
  }
}
